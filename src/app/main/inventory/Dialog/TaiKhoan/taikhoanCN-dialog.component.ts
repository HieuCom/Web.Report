...
export class TaiKhoanCNDialogComponent implements OnInit {
  ...
  async loadDataTaiKhoan() {
    try {
      const data: any = await this.dataService.get("/TaiKhoan").toPromise();
      this.danhSachTaiKhoan = data.filter((item) => item.CN === true);
    } catch (error) {
