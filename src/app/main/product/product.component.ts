...
export class ProductComponent implements OnInit {
  ...
  public search() {
    this._dataService.get('/product/getall?page=' + this.pageIndex + '&pageSize=' + this.pageSize + '&keyword=' + this.filterKeyword + '&categoryId=' + this.filterCategoryID)
  ...
  public showEdit(id: string) {
    this._dataService.get('/product/detail/' + id).subscribe((response: any) => {
  ...
  public delete(id: string) {
    ...
    //   this._dataService.delete('/product/delete', 'id', id).subscribe((response: any) => {
  ...
  private loadProductCategories() {
    this._dataService.get('/productCategory/getallhierachy').subscribe((response: any[]) => {
  ...
  public saveChanges(valid: boolean) {
    if (valid) {
      ...
      if (fi.files.length > 0) {
        this.uploadService.postWithFile('/upload/saveImage?type=product', null, fi.files).then((imageUrl: string) => {
  ...
  private saveData() {
    if (this.entity.ID === undefined) {
      this._dataService.post('/product/add', JSON.stringify(this.entity)).subscribe((response: any) => {
        this.search();
        this.addEditModal.hide();
        this.notificationService.printSuccessMessage(MessageContstants.CREATED_OK_MSG);
      });
    } else {
      this._dataService.put('/product/update', JSON.stringify(this.entity)).subscribe((response: any) => {
        this.search();
        this.addEditModal.hide();
  ...
  public deleteMulti() {
    ...
    // this.notificationService.printConfirmationDialog(MessageContstants.CONFIRM_DELETE_MSG, () => {
    //   this._dataService.delete('/product/deletemulti', 'checkedProducts', JSON.stringify(checkedIds)).subscribe((response: any) => {
    //     this.notificationService.printSuccessMessage(MessageContstants.DELETED_OK_MSG);
    //     this.search();
    //   }, error => this._dataService.handleError(error));
    // });
  ...
  public loadProductImages(id: number) {
    this._dataService.get('/productImage/getall?productId=' + id).subscribe((response: any[]) => {
      this.productImages = response;
    }, error => this._dataService.handleError(error));
  }
  public deleteImage(id: number) {
    // this.notificationService.printConfirmationDialog(MessageContstants.CONFIRM_DELETE_MSG, () => {
    //   this._dataService.delete('/productImage/delete', 'id', id.toString()).subscribe((response: any) => {
    //     this.notificationService.printSuccessMessage(MessageContstants.DELETED_OK_MSG);
    ...
    if (isValid) {
      ...
      if (fi.files.length > 0) {
        this.uploadService.postWithFile('/upload/saveImage?type=product', null, fi.files).then((imageUrl: string) => {
          ...
          this._dataService.post('/productImage/add', JSON.stringify(this.imageEntity)).subscribe((response: any) => {
            this.loadProductImages(this.imageEntity.ProductId);
            this.notificationService.printSuccessMessage(MessageContstants.CREATED_OK_MSG);
          });
        });
  ...
  public loadColors() {
    this._dataService.get('/productQuantity/getcolors').subscribe((response: any[]) => {
      this.colors = response;
    }, error => this._dataService.handleError(error));
  }
  public loadSizes() {
    this._dataService.get('/productQuantity/getsizes').subscribe((response: any[]) => {
      this.sizes = response;
    }, error => this._dataService.handleError(error));
  }

  public loadProductQuantities(id: number) {
    this._dataService.get('/productQuantity/getall?productId=' + id +
  ...
  public saveProductQuantity(isValid: boolean) {
    if (isValid) {
      this._dataService.post('/productQuantity/add', JSON.stringify(this.quantityEntity)).subscribe((response: any) => {
   ...
   public deleteQuantity(productId: number, colorId: string, sizeId: string) {
    ...
    this.notificationService.printConfirmationDialog(MessageContstants.CONFIRM_DELETE_MSG, () => {
      this._dataService.deleteWithMultiParams('/productQuantity/delete', parameters).subscribe((response: any) => {
        this.notificationService.printSuccessMessage(MessageContstants.DELETED_OK_MSG);
